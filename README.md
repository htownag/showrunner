...
Be clear-eyed about this repository's standing as a *public* artifact. The case-law receipts in [`CASE-LAW.md`](CASE-LAW.md) cite artifacts in the private proving grounds through opaque aliases, and the alias→repository legend (`SOURCES.md`) is private by design — the aliases resolve into private repositories, so they cannot be dereferenced from here. Concretely:

- **You can verify the corpus's internal consistency without any private access.** The rows are dated and cross-cited; the principles link to their `CL-###` IDs and those IDs resolve within `CASE-LAW.md`; the mechanisms cite the same receipts the principles do; the enforcement ladder (prose rule → ritual → compiled gate) is legible on its face. A reader can confirm the framework is coherent, that no rule floats free of a cited incident, and that the citations form a closed graph — all from what is published here.
- **You must take the underlying artifacts on the maintainer's word** — *except* where a redacted evidence packet has been published under `evidence/`. That directory is the one place an external reader sees a raw scar (a before-diff excerpt, the first-review verdict, the adversarial findings, the corrected diff), each hygiene-reviewed before publication precisely because redacted diffs are the highest-leak-risk artifact this repo could carry.

"Receipts or it isn't law" (the third extraction test) binds the maintainer *internally* — every rule here really does cite a real, dated incident in the origin. *Externally*, honesty about that boundary — the plain statement that outside the `evidence/` packets you are trusting the maintainer for the artifacts the aliases hide — **is** the receipt. A framework about never lying with evidence does not get to be vague about what its own public evidence proves.

## Adopting this framework in a new project

1. Copy this repository.
2. **Start from [`bindings/MINIMAL.md`](bindings/MINIMAL.md) — the Showrunner Lite profile.** Five questions — what are you building; which files are human-taste territory; what must never be destroyed; what one command proves health; what are your 2–3 lanes — expand into the smallest legal binding, with the defaults already blessed as framework law (merge floor L2, verifier always independent, destructive-op floor absolute — `WORKPLAN.md` "Ballots — RULED 2026-07-10" §4). You fill five blanks, not fifty; a cold reader stands up a working binding from MINIMAL alone in under fifteen minutes. Seed your root `STATUS.md` by copying [`bindings/STATUS-template.md`](bindings/STATUS-template.md) — the day-one human dashboard the director regenerates every shutdown (a first boot before the first shutdown tolerates its absence, but seeding it now closes even that gap).
3. Reach for [`bindings/TEMPLATE.md`](bindings/TEMPLATE.md) — the full declaration, eleven sections of REQUIRED fields — only as your scars grow you into it; do **not** pre-install it. MINIMAL's graduation-by-scar table names, for every section it omits, the exact incident that earns it back (your first design-law ruling → §1; your first two-surfaces-disagree → §6's authority table; your first repeated incident → the failure catalog): graduation into the full apparatus is Principle 4 — "no rule is law until an incident earns it" — applied to adoption itself. Generate the orchestrator and team boot prompts from [`bindings/boot-prompt-template.md`](bindings/boot-prompt-template.md) once you add sessions beyond the first.
...
## Canonical home and downstream sync

This repository is the framework's **canonical home** — the single upstream where the principles, mechanisms, bindings, schema, and tools are authored (the *Home* ballot resolved here 2026-07-08). Adoption step 1 **vendors** a copy into your project; that copy pulls framework updates *from* here and never the other way. A fix you make to a mechanism inside your vendored copy is not a framework change until it is routed back through this repo — never push framework edits out of a project copy directly, or the copies fork and the shared case law quietly diverges. Updates travel up to canonical and back down to every project; they never move sideways between forks.

The boundary between framework and project is the **ledger-data line**. The framework ships the *instruments* — the schema ([`measurement/ledger.schema.json`](measurement/ledger.schema.json)), the tools ([`measurement/ledger.py`](measurement/ledger.py)), the [spec](measurement/review-ledger-spec.md), and a synthetic [`measurement/example-ledger.jsonl`](measurement/example-ledger.jsonl) to demonstrate them — and every project owns its *rows*. A project's real `review-ledger.jsonl` lives project-side and never lands in this repo. Vendor the instruments; keep your own measurements. (This is why the studio project's live 34-row ledger sits in the studio repo, not here.)
